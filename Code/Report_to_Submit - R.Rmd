...
chisq.test_corr = function(data_frame=unicorns_corelation_num, test=chisq.test){
  column_names = colnames(data_frame)
  result_list <- lapply(column_names, function(i) {
    lapply(column_names, function(j) {
      value <- test(data_frame[[i]], 
                    data_frame[[j]])$p.value
      return(data.frame(mesure1 = i, mesure2 = j, p_value = value))
    })
  })
  
  result_df <- do.call(rbind, unlist(result_list, recursive = FALSE))
  return (result_df)
}
...
chisq_test_corr = function(data_frame=unicorns_corelation){
  ...
  result_list <- lapply(column_names, function(i) {
    lapply(column_names, function(j) {
      value <- fisher.test(data_frame[[i]], 
                    data_frame[[j]], )$p.value
      return(data.frame(mesure1 = i, mesure2 = j, p_value = value))
    })
  })
  ...
  result_df <- do.call(rbind, unlist(result_list, recursive = FALSE))
  return (result_df[order(result_df$p_value),])
}
...
fisher_test_corr = function(data_frame=unicorns_corelation){
  ...
  result_list <- lapply(column_names, function(i) {
      ...
      return(data.frame(mesure1 = "target_code", mesure2 = i, p_value = value))
  })
...
unicorns_corelation = unicorns_corelation_num %>% 
...
formatted_cors(unicorns_corelation_num) %>% 
...
```{r, fig.height=6.4, fig.width=8}
formatted_cors(unicorns_corelation_num, "spearman") %>% 
  ggplot(aes(measure1, measure2, fill=r, label=round(r_if_sig,2))) +
  geom_tile() +
...
```{r}
column_names <- colnames(unicorns_corelation_num)
result_list <- lapply(column_names, function(i) {
  lapply(column_names, function(j) {
    value <- chisq.test(factor(unicorns_corelation_num[[i]]), 
                        factor(unicorns_corelation_num[[j]]))$p.value
    return(data.frame(mesure1 = i, mesure2 = j, value = value))
  })
})
result_df_2 <- do.call(rbind, unlist(result_list, recursive = FALSE))

```

```{r}
unicorns_corelation_cat = unicorn_data_complete %>% 
  select(-c(date_added, select_investors, associated_companies, valuation_billions))
colnames(unicorns_corelation_cat)
...
mesure2 = c()
value=c()
for (i in colnames(unicorns_corelation_num)) {
  for (j in colnames(unicorns_corelation_num)) {
    mesure1=c(mesure1, i)
    mesure2=c(mesure2, j)
    value = c(value,
      chisq.test(unicorns_corelation_num[[i]], 
               unicorns_corelation_num[[j]])$p.value
    )
  }
}

colnames(unicorns_corelation_num)
chisq.test(unicorns_corelation_num$country_code, unicorns_corelation_num$target_code)$p.value
...
Now that all our variables are categorical, we have 2 possible tests to compute. Both having the same hypotesis. H0: No link H1: Existing link
...
fisher.test(unicorns_corelation_num$target_code, unicorns_corelation_num$country_code,simulate.p.value = TRUE)
